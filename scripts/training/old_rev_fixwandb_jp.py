...
def trial_suggestions(trial):
    ...
    archlist = [ "facebook/wav2vec2-base",
...
def run(input_pars, modelpath, logpath): 
  ...
  fit_cbs=[
    ...
    EarlyStoppingCallback(comp=np.less, monitor=MONITOR, patience=2),
  ]
  ...
  learn = get_learner(
      ...
      with_attentions=with_attentions,
      cbs=[DropPreds(), SaveModelCallback(fname=arch.replace("/", "_")), ],
      log_cbs=log_cbs,
  )
