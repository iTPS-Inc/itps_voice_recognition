...
def log_predictions(learn, dls, cer, thresh=0.2):
  ...
  dl = dls.test_dl(tfms,
    ...
    after_item=dls.after_item, newline='',
...
class MetricsToWandb(Callback):
    def after_epoch(self):
        ...
        wandb.log({n:s for n,s in zip(self.recorder.metric_names, self.recorder.log) if n not in ['epoch', "time"]})
...
def run(input_pars, modelpath, logpath): 
  ...
  fit_cbs=[
    ...
    SeePreds(arch, tok, n_iters=len(dls.train)//5, log_dir=logdir, neptune=(LOGGING_FRAMEWORK.lower() == "neptune")),
